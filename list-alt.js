...
function addTaskToDOM(taskObj) {
  ...
  span.style.flex = '1';
  span.style.minWidth = '0'; // Allow text to shrink
  ...
  if (taskObj.completed && taskObj.completedAt) {
    timestampDiv.innerHTML = `✅ ${formatTimestamp(taskObj.completedAt)}`;
  ...
  completeBtn.style.minWidth = '35px';
  completeBtn.style.height = '35px';
  ...
  completeBtn.onclick = function() {
    ...
    if (taskObj.completed) {
      ...
      timestampDiv.innerHTML = `✅ ${formatTimestamp(taskObj.completedAt)}`;
      timestampDiv.style.color = '#64f368ff';
      item.style.backgroundColor = 'rgba(100, 243, 104, 0.1)';
    } else {
...
function updateProgress() {
  ...
  if (!progressDisplay) {
    ...
    progressDisplay.style.backgroundColor = "rgba(255, 255, 255, 0.05)";
  ...
  const progressHTML = `
    ...
    <div style="text-align: center; font-size: 0.85em;">
      ...
      ${completedTasks === totalTasks ? `<br><small style="color: #64f368ff; font-size: 0.8em;">All done! 🎉</small>` : ''}
    </div>
  `;
